#!/usr/bin/env bash
# Build every rv32i_m/I/src/*.S file from riscv-arch-test against BOTH the
# Always-Stall-on-Branch 5-stage pipelined DUT (Verilator) and spike (golden reference),
# diff their signatures, and print a PASS/FAIL summary line per file plus a final count.
#
# Usage (from anywhere):
#   PATH="/home/enovo/tools/riscv32-gnu-toolchain/bin:$PATH" \
#   ARCH_TEST_ROOT=~/riscv-arch-test SPIKE_BIN=~/riscv-isa-sim/build/spike \
#     ./run_stallbranch_suite.sh

set -uo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$HERE/../.." && pwd)"
CORE_RTL="$REPO_ROOT/rtl/rv32i_pipelined_stallbranch"
TB="$HERE/tb_arch_test_stallbranch.sv"
...
for SRC in "$SRC_DIR"/*.S; do
  ...
  if is_big "$NAME"; then
    LINK="$LINK_BIG"; MEMW=524288; MAXC=800000
  else
    LINK="$LINK_NORMAL"; MEMW=4096; MAXC=15000
  fi
  ...
  rm -rf obj_dir
  if ! verilator --binary --timing -Wno-fatal --top-module tb_arch_test_stallbranch -Mdir obj_dir \
  ...
  ./obj_dir/Vtb_arch_test_stallbranch > sim.log 2>&1
