`timescale 1ns/1ps
module tb_arch_test_stallbranch #(
    ...
    parameter int MAX_CYCLES = 12000,
    parameter int MEM_WORDS  = 4096
)();
    logic clk = 0;
    logic rst = 1;
    rv32i_pipeline_stallbranch_core #(.MEM_WORDS(MEM_WORDS)) core_dut (.clk(clk), .rst(rst));
    always #5 clk = ~clk;
    integer sigfile;
    integer i;
    initial begin
        $dumpfile("dump.vcd");
        $dumpvars(0, tb_arch_test_stallbranch);
